...
#define WM_VERSION				"20.07.15.1220"
#define WM_DESCRIPTION			"An automative service for CS:GO competition matches"

/* Short match voting */
int g_MenuShowTime = 0;
bool g_playShortmatch = false;
Handle g_countdownTimer = INVALID_HANDLE;

public Plugin myinfo = {
	name = "[RIP AIM CSGO] PUG System (Based on Warmod BFG)",
	author = "Hoan Cadimi",
...
public void OnPluginStart()
{
	...
	wm_short_match_config = CreateConVar("wm_short_match_config", "warmod/ruleset_short.cfg", "Sets the short match config to load on Live on 3");
...
public void GetScoreMsg(int client, char[] result, int maxlen, int t_score, int ct_score) {
	...
	if(g_playShortmatch == false)
	{
		...
		if (t_score > ct_score) {
			Format(result, maxlen, "\x02%t \x04%d\x03-\x04%d", "T Winning SM", g_t_name, t_score, ct_score);
		} else if (t_score == ct_score) {
			Format(result, maxlen, "\x01%t \x04%d\03-\x04%d", "Tied SM", t_score, ct_score);
		} else {
			Format(result, maxlen, "\x0C%t \x04%d\x03-\x04%d", "CT Winning SM", g_ct_name, ct_score, t_score);
		}		
	} 	
}
...
public Action Event_Player_Hurt(Handle event, const char[]name, bool dontBroadcast)
{
	...
	//Code from csgo_damageprint by splewis
	int attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
	int victim = GetClientOfUserId(GetEventInt(event, "userid"));
...
void LiveOn3(bool e_war)
{
	...
	char match_config[64];
	if (g_playShortmatch) {
		GetConVarString(wm_short_match_config, match_config, sizeof(match_config));
...
static void LiveText() {
	if (LiveOn2) {
		...
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t \x03RIP AIM CSGO", chat_prefix, "Powered By");
...
public Action Command_JoinTeam(int client, const char[]command, int args)
{
	...
	GetCmdArg(1, arg, sizeof(arg));
			...
			if (GetTeamClientCount(CS_TEAM_T) >= (max_players/2)) {
				PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t", chat_prefix, "Maximum Players");
				ChangeClientTeam(client, CS_TEAM_SPECTATOR);
				return Plugin_Stop;
			}

			if (GetTeamClientCount(CS_TEAM_CT) >= (max_players/2)) {
				PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t", chat_prefix, "Maximum Players");
				ChangeClientTeam(client, CS_TEAM_SPECTATOR);
				return Plugin_Stop;
			}
...
public Action ChooseTeam(int client, int args)
{
	...
	if ((g_ready_enabled || g_match || g_t_knife || g_max_lock) && max_players != 0 && GetTeamClientCount(CS_TEAM_CT) >= (max_players/2) || GetTeamClientCount(CS_TEAM_T) >= (max_players/2) && CS_GetPlayingCount() >= max_players) {
	//((GetTeamClientCount(CS_TEAM_T) > (max_players/2) && m_iTeam == CS_TEAM_T) || (GetTeamClientCount(CS_TEAM_CT) > (max_players/2) && m_iTeam == CS_TEAM_CT))) {
		PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t", chat_prefix, "Maximum Players");
		ChangeClientTeam(client, CS_TEAM_SPECTATOR);
		return Plugin_Stop;
...
void DispInfo(int client, char[] players_unready, int time)
{
	if (GetConVarBool(wm_ready_panel)) {
		...
		SetGlobalTransTarget(client);
		g_m_ready_up = CreatePanel();
		Format(Temp, sizeof(Temp), "RIP AIM CSGO- %t", "Ready System");
		...
		CloseHandle(g_m_ready_up);
	} else if (!g_live && g_ready_enabled) {
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x04 %t", chat_prefix, "Ready System");
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x04 %t", chat_prefix, "Match Begin Msg", GetConVarInt(wm_min_ready));
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x02 %t\x01%s", chat_prefix, "Info Not Ready", players_unready);
	}
}
...
public Action DisplayHelp(int client, int args)
{
	if (client == 0)
	{
		PrintHintTextToAll("\x05%t: !ready !unready !ws !dao !stickers", "Available Commands");
	}
	else
	{
		if (IsClientConnected(client) && IsClientInGame(client))
		{
			PrintHintText(client, "\x05%t: !ready !unready !ws !dao !stickers", "Available Commands");
		}
	}
	return Plugin_Handled;
}
...
public int voteShortMatchHandle(Menu menu, MenuAction action, int param1, int param2) {
    if (action == MenuAction_End)
    {
    	...
    	if(param1 == 0) {
    		g_playShortmatch = true;
    		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Vote ended. Short match will be started in \x0510 seconds!", chat_prefix);
    		g_countdownTimer = CreateTimer(1.0, Timer_Countdown, _, TIMER_REPEAT);
    	}
    	else
    	{
    		g_playShortmatch = false;
    		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 Vote ended. Default match will be started in \x0510 seconds!", chat_prefix);
    		g_countdownTimer = CreateTimer(1.0, Timer_Countdown, _, TIMER_REPEAT);
    	}
   	}
}
	
		public Action voteShortMatch() {
			if (IsVoteInProgress())
			{
				return Plugin_Handled;
			}
			g_MenuShowTime = 20;
			Menu Menushortmatch = new Menu(voteShortMatchHandle);
			Menushortmatch.SetTitle("Play SHORT MATCH?\nIf the result is NO or TIE, DEFAULT match will be start.");
			Menushortmatch.AddItem("yes", "Yes");
			Menushortmatch.AddItem("no", "No");
			Menushortmatch.ExitButton = false;
			Menushortmatch.DisplayVoteToAll(g_MenuShowTime);
...
public Action Timer_Countdown(Handle timer) {
  ...
  if (warningsPrinted >= 10) {
    ...
    warningsPrinted++;
    int secondsRemaining = 10 - warningsPrinted + 1;
    PrintHintTextToAll("Match will be start in <font color ='#ffff00'>%d seconds<\font>", secondsRemaining);
    return Plugin_Continue;
  }
}